...
🏁 **레이싱 라인 어시스트** (`B` 토글) — 트랙 위에 주행 라인을 그려줍니다.
초록 = 풀스로틀 / 노랑 = 코너 한계 유지 / **빨강 = 브레이킹 구간**, 빨간 이중 바 = 브레이킹 포인트.
라인은 곡률 기반 코너 한계속도에 제동·가속 한계 패스를 적용해 계산하므로 **차종마다 다릅니다**
— F1은 다운포스 덕에 같은 코너를 훨씬 늦게 브레이킹하고, 오벌은 아예 풀스로틀로 돕니다.
인제 서킷의 센터라인은 맵 마스크를 스켈레톤화해 사전 추출했습니다.
